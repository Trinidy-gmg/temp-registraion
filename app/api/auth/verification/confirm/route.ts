...
async function loginAfterVerify(
  email: string,
  password: string
): Promise<Awaited<ReturnType<typeof authBackendPost<LoginTokens | LoginErr>>>> {
  let result = await authBackendPost<LoginTokens | LoginErr>("login", {
    email,
    password,
  });
  /* Rare: mark-verified just committed; retry once if HAMS still sees unverified. */
  if (
    !result.ok &&
    (result.data as LoginErr)?.code === "EMAIL_NOT_VERIFIED" &&
    result.status === 403
  ) {
...
export async function POST(request: Request) {
  try {
    ...
    try {
      loginResult = await loginAfterVerify(payload.email, password);
    } catch (e) {
